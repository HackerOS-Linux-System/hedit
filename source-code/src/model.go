...
type model struct {
	...
	status          string
	quitting        bool
	mode            string // "edit", "search", "gotoline", "prompt", "replace"
	lexer           chroma.Lexer
	theme           *chroma.Style
	...
	replaceStep     int // 0=enter search, 1=enter replace
	hasSelection    bool
	anchorY         int
	anchorX         int
}
...
var (
	...
	// New features
	replaceKey    = key.NewBinding(key.WithKeys("ctrl+r"))   // Find & Replace
	duplicateKey  = key.NewBinding(key.WithKeys("ctrl+d"))   // Duplicate line
	selectAllKey  = key.NewBinding(key.WithKeys("ctrl+a"))   // Select all
	commentKey    = key.NewBinding(key.WithKeys("ctrl+/"))   // Toggle comment
	wordLeftKey   = key.NewBinding(key.WithKeys("ctrl+left")) // Word left
	wordRightKey  = key.NewBinding(key.WithKeys("ctrl+right"))// Word right
...
// ================================================
// NEW FEATURE: Duplicate current line
// ================================================
func (m *model) duplicateLine() {
	line := m.lines[m.cursorY]
	m.lines = append(m.lines[:m.cursorY+1], append([]string{line}, m.lines[m.cursorY+1:]...)...)
	m.pushUndo(action{kind: "deleteLines", y: m.cursorY + 1, lines: []string{line}})
	m.cursorY++
	m.updateLineNumWidth()
...
// ================================================
// NEW FEATURE: Toggle line comment (// for Go/C/JS, # for Python/shell)
// ================================================
func (m *model) toggleComment() {
	prefix := "// "
	fname := m.filename
	if strings.HasSuffix(fname, ".py") || strings.HasSuffix(fname, ".sh") ||
		strings.HasSuffix(fname, ".bash") || strings.HasSuffix(fname, ".rb") ||
		strings.HasSuffix(fname, ".yaml") || strings.HasSuffix(fname, ".yml") ||
		strings.HasSuffix(fname, ".toml") {
		prefix = "# "
	} else if strings.HasSuffix(fname, ".lua") {
		prefix = "-- "
	} else if strings.HasSuffix(fname, ".sql") {
		prefix = "-- "
	} else if strings.HasSuffix(fname, ".html") || strings.HasSuffix(fname, ".xml") {
		// HTML/XML: skip for now, use // as fallback
		prefix = "<!-- "
	}
	...
	if strings.HasPrefix(trimmed, prefix) {
		// remove comment
		newLine := indent + trimmed[len(prefix):]
		m.pushUndo(action{kind: "insert", y: m.cursorY, x: len(indent), text: prefix})
		m.lines[m.cursorY] = newLine
		if m.cursorX > len(prefix) {
			m.cursorX -= len(prefix)
		}
	} else {
		// add comment
		m.pushUndo(action{kind: "delete", y: m.cursorY, x: len(indent), text: prefix})
		m.lines[m.cursorY] = indent + prefix + trimmed
		m.cursorX += len(prefix)
	}
	m.invalidateCache(m.cursorY)
	m.modified = true
...
func (m *model) doReplace() {
	phrase := m.searchPhrase
	repl := m.replacePhrase
	if phrase == "" {
		return
	}
	count := 0
	for i := range m.lines {
		newLine := strings.ReplaceAll(m.lines[i], phrase, repl)
		if newLine != m.lines[i] {
			count += strings.Count(m.lines[i], phrase)
			m.lines[i] = newLine
			m.invalidateCache(i)
		}
	}
	...
	case tea.KeyMsg:
		...
		if m.mode == "replace" {
			...
			if m.replaceStep == 0 {
				...
				if msg.String() == "enter" {
					m.searchPhrase = m.searchInput.Value()
					m.replaceStep = 1
				...
				if msg.String() == "enter" {
					m.replacePhrase = m.replaceInput.Value()
					m.doReplace()
					m.mode = "edit"
					m.replaceStep = 0
		...
		case key.Matches(msg, replaceKey):
			m.mode = "replace"
			m.replaceStep = 0
			m.searchInput.Placeholder = "Find: "
			m.searchInput.SetValue("")
			m.searchInput.Focus()
			return m, nil

		// NEW: Duplicate line
		case key.Matches(msg, duplicateKey):
			m.duplicateLine()
			m.adjustScroll()
			return m, nil
		...
		// NEW: Toggle comment
		case key.Matches(msg, commentKey):
			m.toggleComment()
			m.adjustScroll()
			return m, nil

		// NEW: Word left (Ctrl+Left)
		case key.Matches(msg, wordLeftKey):
			m.cursorX = wordStart(m.lines[m.cursorY], m.cursorX)
			m.targetVisualCol = visualCol(m.lines[m.cursorY], m.cursorX)
			m.adjustScroll()
			return m, nil

		// NEW: Word right (Ctrl+Right)
		case key.Matches(msg, wordRightKey):
			m.cursorX = wordEnd(m.lines[m.cursorY], m.cursorX)
			m.targetVisualCol = visualCol(m.lines[m.cursorY], m.cursorX)
			m.adjustScroll()
			return m, nil
		...
		if shift && !m.hasSelection {
			...
			m.anchorX = m.cursorX
			m.hasSelection = true
		} else if !shift && m.hasSelection && msg.Type != tea.KeyRunes {
			// only clear selection on non-typing keys
			m.hasSelection = false
		}
		...
		case tea.KeyHome, tea.KeyCtrlA:
			// Ctrl+A is now select-all, handled above; Home still works
			m.cursorX = 0
			m.targetVisualCol = visualCol(m.lines[m.cursorY], m.cursorX)
